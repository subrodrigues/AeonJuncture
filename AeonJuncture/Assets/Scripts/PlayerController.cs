			...
			if (isAttacking && !MaxAnimationTimeReached ()) {
				...
				if (Input.touchCount > 0) {
					...
					if (!isAttacking && Input.touchCount == 1
						...
						// Se toca no Mana Knight
						for (int i = 0; i < manaKnight.Length; i++) {
							if (manaKnight [i].collider.Raycast (ray, out hit, Mathf.Infinity)) {
								manaKnight [i].transform.SendMessage ("SetInitTracer", true, SendMessageOptions.DontRequireReceiver);
							}
						}
						if (manaKnightWithKey.collider.Raycast (ray, out hit, Mathf.Infinity)) {
							manaKnightWithKey.transform.SendMessage ("SetInitTracer", true, SendMessageOptions.DontRequireReceiver);
						}
				...
				if (touchingQuicksand) {
					...
					if (transform.position.y <= bellowGround) {
						touchingQuicksand = false;
						ApplyDamage (10.0f);
					}
				}
